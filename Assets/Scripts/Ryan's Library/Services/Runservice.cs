...
public sealed class Runservice : Singleton {
	...
	/// <summary>
	/// Runs an action after a set amount of time. Does not repeat unlike RunAfter with Func.
	/// </summary>
	/// <param name="priority"></param>
	/// <param name="length"></param>
	/// <param name="func"></param>
	/// <returns>A Listener object.</returns>
	public static Listener<float> RunAfter(int priority, float time, Action func) {
		float counter = 0;
		return BindToEither(priority, (float dt) => {
			if (counter >= time) {
				func.Invoke();
	...
	/// <summary>
	/// Binds a function to run every set amount of time.
	/// </summary>
	/// <param name="priority"></param>
	/// <param name="length"></param>
	/// <param name="func"></param>
	...
	public static Listener<float> RunEvery(int priority, float timer, Action func) {
		float counter = 0;
		return BindToEither(priority, (float dt) => {
			counter += dt;
			if (counter >= timer) {
	...
	private static void BeforeCameraRender(Camera cam) {
		UpdateEither(_RenderStepFuncList, (Time.realtimeSinceStartup - PrevTime_RenderStep));
