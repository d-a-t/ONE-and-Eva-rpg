...
public sealed class InputController : Singleton {
	public static Variable<KeyCode> LastKey = new Variable<KeyCode>();
	...
	public void Start() {
		foreach (KeyCode vKey in System.Enum.GetValues(typeof(KeyCode))) {
			InputController.Keyboard[vKey].Value = false;
		}

		Listener<float> updateMouse = Runservice.BindToUpdate(1, (dt) => {
		...
		Listener<float> updateKeyCodes = Runservice.BindToUpdate(1, (dt) => {
		...
		Listener<float> updateKeyStroke = Runservice.BindToUpdate(1, (dt) => {
